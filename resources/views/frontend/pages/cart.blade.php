            ...
            @forelse ($cartItems as $cartItem)
                  ...
                  <div class="d-flex align-items-center gap-3">
                    <div class="cart-item__thumb">
                      @if($cartItem->item->preview_type == 'image')
                        <a href="{{ route('products.show', $cartItem->item->slug) }}" class="link">
                      ...
                      @elseif($cartItem->item->preview_type == 'video')
                        <a href="{{ route('products.show', $cartItem->item->slug) }}" class="link">
                          ...
                          class="cover-img" style="height: 130px; width: 164px">
                        </a>
                    ...
                    <div class="cart-item__content">
                      ...
                      <span class="cart-item__price font-18 text-heading fw-500">
                        Category:
                        <span class="text-body font-14 text-primary">
                          {{ $cartItem->item->category->name }} / {{ $cartItem->item->subCategory->name }}
                        </span>
                      </span>
                    </div>
                  </div>
              ...
              <td class="price">
                @if($cartItem->item->discount_price > 0)
                  <span class="cart-item__totalPrice font-18 text-heading fw-500">${{ $cartItem->item->discount_price }}</span>
                  <span class="cart-item__totalPrice text-body font-18 fw-400 text-decoration-line-through" style="color: red !important;">${{ $cartItem->item->price }}</span>
                @else
                  <span class="cart-item__totalPrice font-18 text-heading fw-500 mb-0">${{ $cartItem->item->price }}</span>
                @endif
              </td>
